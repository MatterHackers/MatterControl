...
namespace MatterHackers.MatterControl.ActionBar
{
    class PrinterActionRow : ActionRowBase
    {
        ...
        string DisconnectingWillCancelTitle = "Disconnect and cancel the current print?".Localize();
        string DisconnectingWillCancelMessage = "WARNING: Disconnecting will cancel the current print.\n\nDo you want to disconnect?".Localize();
        void OnIdleDisconnect(object state)
        {
            bool doCancel = true;
            if (PrinterCommunication.Instance.PrinterIsPrinting)
            {
                if (StyledMessageBox.ShowMessageBox(DisconnectingWillCancelMessage, DisconnectingWillCancelTitle, StyledMessageBox.MessageType.YES_NO))
                {
